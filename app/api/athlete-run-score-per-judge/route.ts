...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const runNumParam = req.nextUrl.searchParams.get("run_num");
    if (!runNumParam) {
      return NextResponse.json(
        { error: "Missing the run number identifier" },
        { status: 400 }
      );
    }
    const runNum = parseInt(runNumParam);


    const athleteIdParam = req.nextUrl.searchParams.get("athlete_id");
    if (!athleteIdParam) {
      return NextResponse.json(
        { error: "Missing athlete identifier" },
        { status: 400 }
      );
    }
