    ...
    return (
        ...
        <div className="space-y-6 p-4 md:pt-2 md:px-6 md:pb-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-base-300"> {/* Reduced pb */}
                ...
                <Link href="/admin/events" className="btn btn-sm btn-outline self-start sm:self-center whitespace-nowrap">
                    Back to Events List
                </Link>
            </div>
            ...
            <div className="card bg-base-100 shadow-xl">
                <div className="card-body">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                        <p><strong className="font-medium text-base-content/70 block mb-0.5">Location:</strong> {event.location}</p>
                        <p><strong className="font-medium text-base-content/70 block mb-0.5">Dates:</strong> {formatDateRange(startDate, endDate)}</p>
                        <p><strong className="font-medium text-base-content/70 block mb-0.5">Status:</strong>
                        ...
                        {event.headJudge && event.headJudge.length > 0 && (
                             <p className="md:col-span-2"><strong className="font-medium text-base-content/70 block mb-0.5">Head Judge(s):</strong> {event.headJudge.map(hj => `${hj.first_name} ${hj.last_name}`).join(', ')}</p>
                        )}
                    </div>
                </div>
            </div>
            ...
            {event.status?.toLowerCase() === 'draft' && (
                <div className="mt-8 pt-6 border-t border-base-300 text-center">
                    ...
                    <p className="text-xs text-base-content/60 mt-2">Make this event visible to the public and open for registrations (if applicable).</p>
                </div>
            )}
        </div>
    );
