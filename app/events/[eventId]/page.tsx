// app/events/[eventId]/page.tsx
import React from 'react';
// Link component is not strictly needed if the only link is the admin one using <a>
// import Link from 'next/link';
import { fetchEventById } from '@/lib/data';
import { formatDate, formatDateRange } from '@/lib/utils'; // Assuming utils.ts for formatters
import type { EventDetails } from '@/lib/definitions';        // Import from your centralized definitions
import { notFound } from 'next/navigation';
// Import auth to get the userId, and your custom function to get app-specific user details
import { auth } from '@clerk/nextjs/server';
import { getAuthenticatedUserWithRole } from '@/lib/auth/user'; // Using your specified path
import type { AppUserWithRole } from '@/lib/auth/user';      // Type from your auth/user.ts
import type { Metadata } from 'next';
import BlankHeader from '@/components/blankHeader'; // Using your specified public header
import { CalendarDaysIcon, MapPinIcon, FlagIcon, TrophyIcon, UsersIcon, ShieldCheckIcon, InformationCircleIcon } from '@heroicons/react/24/outline';

// Props type for the page component
type PublicEventDetailPageProps = {
    params: { eventId: string };
};

// Function to generate dynamic metadata for SEO and browser tab
export async function generateMetadata({ params }: PublicEventDetailPageProps): Promise<Metadata> {
    const eventId = Number(params.eventId);

    if (isNaN(eventId)) {
        return { title: 'Event Not Found | SnowScore' };
    }

    const event = await fetchEventById(eventId);

    if (!event) {
        return { title: 'Event Not Found | SnowScore' };
    }

    // Ensure dates are valid for formatting in description
    const startDateForDesc = event.start_date instanceof Date ? event.start_date : new Date(event.start_date);
    const endDateForDesc = event.end_date instanceof Date ? event.end_date : new Date(event.end_date);

    return {
        title: `${event.name} | Event Details | SnowScore`,
        description: `View details for the event: ${event.name}, taking place at ${event.location} from ${formatDate(startDateForDesc)} to ${formatDate(endDateForDesc)}.`,
        openGraph: {
            title: `${event.name} | SnowScore`,
            description: `Event details for ${event.name}.`,
            type: 'article',
        },
    };
}

// The main page component
export default async function PublicEventDetailPage({ params }: PublicEventDetailPageProps) {
    const eventId = Number(params.eventId);

    if (isNaN(eventId)) {
        console.warn(`PublicEventDetailPage: Invalid eventId param received: ${params.eventId}`);
        notFound();
    }

    const event: EventDetails | null = await fetchEventById(eventId);

    if (!event) {
        console.warn(`PublicEventDetailPage: Event not found in database for ID: ${eventId}`);
        notFound();
    }

    // --- ADMIN CHECK USING DATABASE ROLE ---
    const authResult = await auth();
    const clerkUserId = authResult.userId;

    let isAdmin = false;
    if (clerkUserId) {
        const appUser: AppUserWithRole | null = await getAuthenticatedUserWithRole();
        if (appUser) {
            console.log(`PublicEventDetailPage: Fetched app user via getAuthenticatedUserWithRole: ${appUser.email}, DB Role: ${appUser.roleName}`);
            const adminRoles = ['admin', 'Executive Director', 'Administrator', 'Chief of Competition'];
            isAdmin = adminRoles.includes(appUser.roleName);
        } else {
            console.warn(`PublicEventDetailPage: User ${clerkUserId} authenticated with Clerk but no corresponding user/role found in application database.`);
        }
    }
    // --- END ADMIN CHECK ---

    const startDate = event.start_date instanceof Date ? event.start_date : new Date(event.start_date);
    const endDate = event.end_date instanceof Date ? event.end_date : new Date(event.end_date);

    return (
        <main className="bg-base-200 min-h-screen">
            <BlankHeader />

            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
                <div className="bg-base-100 p-6 md:p-10 rounded-2xl shadow-xl">
                    {/* Event Header */}
                    <div className="border-b border-base-300 pb-6 mb-8">
                        <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                            <div className="flex-grow">
                                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-primary tracking-tight leading-tight">
                                    {event.name}
                                </h1>
                                <p className="mt-2 text-lg sm:text-xl text-base-content opacity-80 flex items-center">
                                    <MapPinIcon className="h-5 w-5 mr-2 opacity-70" />
                                    {event.location}
                                </p>
                            </div>
                            {/* Admin Management Link - uses <a> tag for full page reload */}
                            {isAdmin && (
                                <a // Using <a> tag to ensure full page reload to admin layout
                                    href={`/admin/events/${eventId}`}
                                    className="btn btn-accent btn-sm mt-2 sm:mt-0 self-start sm:self-auto whitespace-nowrap"
                                    title="Manage this event in the admin panel"
                                >
                                    <ShieldCheckIcon className="h-4 w-4 mr-1" />
                                    Admin: Manage Event
                                </a>
                            )}
                        </div>
                    </div>

                    {/* Core Event Info Grid */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
                        <InfoCard icon={<CalendarDaysIcon className="h-6 w-6 text-secondary" />} title="Dates" value={formatDateRange(startDate, endDate)} />
                        <InfoCard icon={<FlagIcon className="h-6 w-6 text-secondary" />} title="Status" value={event.status || 'N/A'} badgeClass={
                            event.status?.toLowerCase() === 'scheduled' ? 'badge-success' :
                            event.status?.toLowerCase() === 'completed' ? 'badge-primary' :
                            event.status?.toLowerCase() === 'cancelled' ? 'badge-error' : 'badge-ghost'
                        } isBadge={true} />
                        <InfoCard icon={<TrophyIcon className="h-6 w-6 text-secondary" />} title="Discipline" value={event.discipline_name || 'Not Specified'} />
                    </div>

                    {/* Divisions Section */}
                    {event.divisions && event.divisions.length > 0 && (
                        <Section title="Event Divisions">
                            <div className="flex flex-wrap gap-3">
                                {event.divisions.map((division) => (
                                    <span key={division.division_id} className="badge badge-lg badge-outline border-base-content/30 text-base-content py-3 px-4">
                                        {division.division_name}
                                    </span>
                                ))}
                            </div>
                        </Section>
                    )}

                    {/* Registered Athletes Section (Public View) */}
                    {event.athletes && event.athletes.length > 0 && (
                        <Section title="Registered Participants">
                            <div className="overflow-x-auto bg-base-200/30 p-4 rounded-lg">
                                <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-2">
                                    {event.athletes.map(athlete => (
                                        <li key={athlete.athlete_id} className="text-base-content py-1 flex items-center">
                                            <UsersIcon className="h-4 w-4 mr-2 opacity-60 flex-shrink-0" />
                                            <span>{athlete.first_name} {athlete.last_name}
                                            {athlete.bib_num && <span className="text-xs opacity-70 ml-1">(Bib #{athlete.bib_num})</span>}
                                            </span>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                        </Section>
                    )}
                    {(!event.athletes || event.athletes.length === 0) && (
                         <Section title="Participants">
                            <div className="flex items-center text-base-content/70 italic p-4 bg-base-200/30 rounded-lg">
                                <InformationCircleIcon className="h-5 w-5 mr-2"/>
                                Participant information will be available closer to the event or as registrations are confirmed.
                            </div>
                        </Section>
                    )}
                </div>
            </div>
        </main>
    );
}

// Helper component for consistent info card styling
const InfoCard: React.FC<{ icon: React.ReactNode; title: string; value: string; isBadge?: boolean; badgeClass?: string }> = ({ icon, title, value, isBadge, badgeClass }) => (
    <div className="bg-base-200/40 p-4 rounded-lg shadow">
        <div className="flex items-center mb-1">
            {icon}
            <h3 className="ml-2 text-sm font-semibold uppercase text-base-content/70 tracking-wider">{title}</h3>
        </div>
        {isBadge ? (
            <p className="mt-1 text-lg">
                <span className={`badge badge-md ${badgeClass || 'badge-ghost'} badge-outline`}>{value}</span>
            </p>
        ) : (
            <p className="mt-1 text-lg text-base-content font-medium">{value}</p>
        )}
    </div>
);

// Helper component for consistent section styling
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <div className="mb-8 md:mb-10">
        <h2 className="text-2xl font-semibold mb-4 text-secondary border-b border-base-300 pb-2">
            {title}
        </h2>
        {children}
    </div>
);