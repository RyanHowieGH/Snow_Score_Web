...
export default function ManageJudgingPanelsDisplay({
  event_id,
  panels,
}: JudgingPanelPerEventProps) {
  ...
  const refreshPanels = async () => {
    const response = await fetch(`/api/updated-qr-code?event_id=${event_id}`);
    if (!response.ok) {
      console.error("Failed to fetch updated panels");
      return;
    }

    const updatedPanels: JudgingPanelPerEvent[] = await response.json();
    setRenderedPanels(updatedPanels);
  };
  ...
  return (
    <div className="space-y-6">
      {Object.entries(panelsMap).map(
        ([divisionId, { divisionName, rounds }]) => (
            ...
            <div className="card-body space-y-4">
              ...
              {Object.entries(rounds).map(([roundId, { roundName, heats }]) => (
                <div key={roundId} className="space-y-3">
                  <h3 className="font-semibold text-secondary text-2xl mt-5">
                    ROUND: {roundName}
                  </h3>
